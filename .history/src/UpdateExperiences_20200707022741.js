...
export default function AddExperiences() {
  ...
  useEffect(() => {
    ...
    axios.get(process.env.REACT_APP_GET_EXP + `/${expId}`).then((res) => console.log(res.data.data.experience));
  }, [expId]);
  ...
  return (
    <Container>
      <Form>
        ...
        <Form.Group>
          <Form.File label="Experience images*" onChange={(e) => setImages(e.target.files)} value={exp.images} multiple />
        </Form.Group>
