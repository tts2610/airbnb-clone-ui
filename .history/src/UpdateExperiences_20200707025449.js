...
export default function AddExperiences() {
  ...
  useEffect(() => {
    ...
    axios.get(process.env.REACT_APP_GET_EXP + `/${expId}`).then(function (res) {
      const { title, description, tags, language, duration, price, images, location, isLoading, groupSize } = res.data.data.experience;
      setTitle(title);
      ...
      <Form>
        <Form.Row>
          ...
          <Form.Group as={Col} controlId="formGridState">
            ...
            <Form.Control as="select" onChange={(e) => setLanguage(e.target.value)} defaultValue={exp.language}>
        ...
        <Form.Group>
          <Form.Row style={{ marginBottom: "-31px", marginTop: "23px" }}>
            <div>Tags:</div>
            <TagInput
              tags={exp.tags.map(function (e, indx) {
