...
export default function AddExperiences() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!title || !description || !duration || !price || !location || !groupSize) {
    ...
    //   }
    // formData.append("title", "Truogn Tuan Son");
    ...
    const exp = {
      ...
      groupSize: groupSize,
      tags: oldTags.map((element) => element.displayValue),
      location: location,
    };
    ...
    axios
      .put(process.env.REACT_APP_ADD_EXP + `/${expId}`, exp, { headers: headers })
      .then((res) => {
        console.log(res);
